...
const returnOrder = async (req, res) => {
     ...
     try {
          ...
          const returnOrder = await prisma.retailer_return_wholesaler.create({
               data: {
                    return_reason,
                    return_description,
                    order_id,
          ...
          const sales = await prisma.wholesaler_Sold.findMany({
               where: {
                   order_id: order_id,
               },
          });
          ...
          // updating medicine stock after returning the order from the retailer 
          for(let i=0; i<order.order_items.length; i++){
               const medi = order.order_items[i];
          ...
          // updating bought history of the retailer after returning the order
          for(let i=0; i<order.order_items.length; i++){
               const item = order.order_items[i];
               await prisma.retailer_Bought.update({
